...
def estimate_cbh_proxy(
    ...
    """
    Estimate canopy base height from per-cell vertical point profiles.

    The proxy is the lowest occupied vertical bin that begins a sustained canopy
    column. Sparse cells remain nodata. This is a LiDAR structural proxy, not a
    modeled fuel attribute.
    """
    ...
    if histogram.ndim != 3:
        raise ValueError("vertical profile histogram must be a 3-D array")

    rows, cols, bin_count = histogram.shape
    if len(bin_edges) != bin_count + 1:
        raise ValueError("bin_edges length must be one greater than histogram depth")

    bin_bottoms = bin_edges[:-1].astype(np.float32)
    min_start_bin = int(np.searchsorted(bin_bottoms, min_canopy_height, side="left"))
    min_start_bin = min(max(min_start_bin, 0), bin_count)
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            profile = occupied[row, col]
            start = _first_sustained_canopy_bin(
                profile,
                min_start_bin,
                gap_tolerance_bins,
            ...
            if start is not None:
                cbh[row, col] = bin_bottoms[start]

    return cbh


def _first_sustained_canopy_bin(
    occupied_profile,
    min_start_bin,
    gap_tolerance_bins,
    min_canopy_depth_bins,
):
    candidate_bins = np.flatnonzero(occupied_profile[min_start_bin:]) + min_start_bin
    if candidate_bins.size == 0:
        return None

    for start in candidate_bins:
        occupied_seen = 0
        gaps_seen = 0

        for bin_idx in range(start, len(occupied_profile)):
